...
\begin{table}[htbp]
	\begin{tabular}{| p{3cm} || p{2cm} | p{2cm} | p{2cm} | p{2cm} |} \hline
		...
		Modularität & x & x & x & x \\ \hline
		Erweiterbarkeit zur Laufzeit & x & x & x & x \\ \hline
		dynamisch konfigurierbare Orchestirierung & x & x & x & x \\ \hline
		Skalierbarkeit & x & x & x & x \\ \hline
		Effizienz bei der Weiterleitung von Daten & x & x & x & x \\ \hline
		Lizenz & x & x & unbekannt & Eigen\-entwicklung \\ \hline
	\end{tabular}
...
Überstrahlte Bereiche können auftreten, wenn sich externe Lichtquellen direkt über der Tischoberfläche befinden oder bei internen (im Werkzeug eingebauten) Lichtquellen keine Streuscheiben verwendet werden. Außerdem kann auch durch direkte Sonneneinstahlung ein Überstrahlen auftreten. In Abgrenzung zu "Spotlights" sind überstrahlte Bereiche großflächiger (potentiell die gesamte Oberfläche) und werden nicht als Cursor erkannt. Überstrahlte Bereiche verursachen jedoch andere Probleme, die im Folgenden behandelt werden.
